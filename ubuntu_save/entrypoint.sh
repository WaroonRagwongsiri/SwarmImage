...
if [ ! -f "$PVC_BASHRC" ]; then
	...
	# Optional: Inject Dockerfile ENV paths into .bashrc so they survive SSH Swarm overwrites
	echo 'export PATH="/usr/local/cuda/bin:/usr/local/nvm/versions/node/v22/bin:$PATH"' >> "$PVC_BASHRC"
	echo 'export LD_LIBRARY_PATH="/usr/local/cuda/lib64:$LD_LIBRARY_PATH"' >> "$PVC_BASHRC"
fi
